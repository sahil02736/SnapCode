...
export default function MyCodeEditor() {
  ...
  useEffect(() => {
    const randomSnippet =
      ...
      // const testa = (codeSnippets[Math.floor(Math.random() * codeSnippets.length)])
      // console.log(codeSnippets[Math.floor(Math.random() * codeSnippets.length)].code)
    ...
    >
      ...
      >
        ...
        <CodeEditor
          ...
          value={store.code}
          language={store.language}
          onChange={(evn) => useStore.setState({code : evn.target.value})}
          padding={15}
          style={{
